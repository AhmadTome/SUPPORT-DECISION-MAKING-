...
<html class="fixed">
<head>
    <title> عدد الطلاب الزائرين في كل فصل</title>
    <!-- Basic -->
    <meta charset="UTF-8">
...
// Check connection
    ...
    $query = "SELECT CONCAT(`semester`,`year`) as visit_date , sum(`numberOfStudent`) as count FROM `visit` group by `semester`,`year` ORDER by year desc";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            array_push($info,
                ["count" => $row["count"],"visit_date" => $row["visit_date"]
                ]);
        }
        return $info;
        //echo $sl_number;
